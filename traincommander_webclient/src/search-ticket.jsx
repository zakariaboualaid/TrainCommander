...
module.exports = React.createClass({
	...
	startPaypalTransaction: function(trip_id){
		Api.initiateOrder(trip_id).then(function(data){
			this.setState({order: data});
			$(".form_order_trip input[name='return']").val("http://"+Functions.getHost()+"/my-orders?order_id="+this.state.order.id)
			$(".form_order_trip").submit();
		}.bind(this));
	},
	...
	returnTripsTable: function(){
		var trips = this.state.trips.map(function(trip){
			return <tr key={trip.id}>
					...
					<td>
						<React.Bootstrap.Form className="form_order_trip" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
						...
						<a href="#" onClick={this.startPaypalTransaction.bind(this, trip.id)} className="btn btn-link btn-xs">
							<input type="image" src="../img/paypal_please.png" border="0" width="110px" alt="PayPal - The safer, easier way to pay online!"/>
						</a>
					</td>
				</tr>
		}.bind(this))
